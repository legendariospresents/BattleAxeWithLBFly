...
void ButtonInfo::calculateSize(const char* txt) {
	std::string str(txt);
	this->size = {DrawUtils::getTextWidth(&str), DrawUtils::getFont(Fonts::RUNE)->getLineHeight()};
}
...
void ButtonInfo::draw(vec2_t mousePos, const char* label) {
	...
	if (centered) {
		...
		textPos.y -= DrawUtils::getFont(Fonts::RUNE)->getLineHeight() / 2;
	}
		
	DrawUtils::drawText(textPos, &str, MC_Color());
	if (isInSelectableSurface(mousePos)) {  // Mouse hovering over us
		DrawUtils::fillRectangle(surface, MC_Color(37, 74, 28), 1);
		this->canClickB = true;
	} else {
		DrawUtils::fillRectangle(surface, MC_Color(32, 32, 32), 1);
