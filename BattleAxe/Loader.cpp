...
DWORD WINAPI injectorConnectionThread(LPVOID lpParam) {
	...
	auto** battleToInjectorPtr = reinterpret_cast<MemoryBoi**>(magicArray + sizeof(magicValues));
	auto** injectorToBattlePtr = reinterpret_cast<MemoryBoi**>(magicArray + sizeof(magicValues) + sizeof(uintptr_t));

	*battleToInjectorPtr = new MemoryBoi();
	MemoryBoi* battleToInjector = *battleToInjectorPtr;
	battleToInjector->isPresent = true;
	battleToInjector->protocolVersion = 1;

	*injectorToBattlePtr = new MemoryBoi();
	MemoryBoi* injectorToBattle = *injectorToBattlePtr;

	magicArray[0] = 0x48;  //Only find this allocated one, not the one in the thread stack

	logF("battleToInjectorPtr at %llX", battleToInjector);
	logF("injectorToBattlePtr at %llX", injectorToBattle);
	...
	while (isRunning) {
		...
		bool isConnected = battleToInjector->isPresent && injectorToBattle->isPresent && battleToInjector->protocolVersion >= injectorToBattle->protocolVersion;

		if (isConnected && !injectorToBattle->isUnread) {
			...
			if (realElapsed > 4.f) {
				...
				injectorToBattle->isPresent = false;
				QueryPerformanceCounter(&timeSinceLastMessage);
			}
		}
		...
		if (isConnected) {
			...
			if (injectorToBattle->isUnread) {  // They sent us a message
				QueryPerformanceCounter(&timeSinceLastMessage);
				switch (injectorToBattle->cmd) {
				case CMD_INIT: {
					logF("Got CMD_INIT from injector");
					loggedIn = true;
					int flags = injectorToBattle->params[0];
					if (flags & (1 << 0) && injectorToBattle->dataSize > 0 && injectorToBattle->dataSize < sizeof(injectorToBattle->data)) {  // Has Json data
						injectorToBattle->data[sizeof(injectorToBattle->data) - 1] = '\0';
						json data = json::parse(reinterpret_cast<char*>(injectorToBattle->data));
				...
				case CMD_PING: {
					BattleDataPacket pongPacket;
					pongPacket.cmd = CMD_PONG;
					pongPacket.params[0] = injectorToBattle->params[0];
				...
				case CMD_RESPONSE: {
					int id = injectorToBattle->params[1];

					auto pk = std::make_shared<BattleDataPacket>();
					pk->cmd = (DATAPACKET_CMD)injectorToBattle->params[0];
					memcpy(pk->params, &injectorToBattle->params[2], sizeof(injectorToBattle->params) - sizeof(int) * 2);

					if (injectorToBattle->dataSize > 0 && injectorToBattle->dataSize < 3000) {
						pk->dataArraySize = injectorToBattle->dataSize;

						auto dataTemp = new unsigned char[injectorToBattle->dataSize + 2];
						memset(dataTemp + injectorToBattle->dataSize, 0, 2);  // If we don't zero the last 2 bytes, printing as unicode string won't work
						memcpy(dataTemp, injectorToBattle->data, injectorToBattle->dataSize);
				...
				case CMD_LOG: {
					if(injectorToBattle->dataSize > 4 && injectorToBattle->dataSize < 2999){
						injectorToBattle->data[injectorToBattle->dataSize] = 0; // null terminator

						char* command = reinterpret_cast<char*>(&injectorToBattle->data[3]);
				...
				injectorToBattle->isUnread = false;
			}
			...
			if (!battleToInjector->isUnread && !g_Data.isPacketToInjectorQueueEmpty()) {
				QueryPerformanceCounter(&timeSinceLastMessage);
				// They read the message, lets send the next one
				BattleDataPacket nextDataPack = g_Data.getPacketToInjector();
				if (nextDataPack.dataArraySize >= 3000) {
					throw std::exception("The Data packet is too big to send");
				}

				battleToInjector->cmd = nextDataPack.cmd;
				memcpy(battleToInjector->params, nextDataPack.params, sizeof(int) * 5);
				if (nextDataPack.dataArraySize > 0) {
					memcpy(battleToInjector->data, nextDataPack.data.get(), nextDataPack.dataArraySize);
				}

				battleToInjector->dataSize = nextDataPack.dataArraySize;

				battleToInjector->isUnread = true;
			}
		} else
			Sleep(30);
	}
	logF("Quitting connection thread");
	battleToInjector->isPresent = false;
	memset(magicValues, 0, sizeof(magicValues));
	memset(magicArray, 0, sizeof(magicValues) + sizeof(uintptr_t) * 2);
	Sleep(150);
	delete *battleToInjectorPtr;
	delete *injectorToBattlePtr;
	delete[] magicArray;
