...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	DrawUtils::setCtx(renderCtx, dat);
	{
		...
		if (!g_Data.isInjectorConnectionActive()) {
			...
			if (elapsedFlot > 1.5f) {
				...
				std::string text = "Download the new injector!";
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			// Draw BIG watermark
			{
				...
				DrawUtils::fillRectangle(rectPos, MC_Color(32, 32, 32, 1), 1.f);
			...
			if (g_Data.allowWIPFeatures()) {
				if (HImGui.Button("Load Script Folder", vec2_t(wid.x * (0.765f - 0.5f), wid.y * 0.92f), true)) {
					...
					packet.params[0] = g_Data.addInjectorResponseCallback([](std::shared_ptr<BattleDataPacket> pk) {
...

			{
				...
				if (shouldRenderWatermark) {
					...
					static const float textHeight = (nameTextSize + versionTextSize * 0.7f /* We don't quite want the version string in its own line, just a bit below the name */) * DrawUtils::getFont(Fonts::RUNE)->getLineHeight();
	...
	// Draw Info / Alert Boxes
	{
		...
		if (box) {
			...
			const float titleTextHeight = DrawUtils::getFont(Fonts::RUNE)->getLineHeight() * titleTextSize;
			...
			const float messageHeight = DrawUtils::getFont(Fonts::RUNE)->getLineHeight() * messageTextSize * lines;
			...
			DrawUtils::fillRectangle(rectPos, MC_Color(32, 32, 32), box->fadeVal);
...
void Hooks::ClickFunc(__int64 a1, char mouseButton, char isDown, __int16 mouseX, __int16 mouseY, __int16 relativeMovementX, __int16 relativeMovementY, char a8) {
	...
	static auto clickGuiModule = moduleMgr->getModule<ClickGuiMod>();
	//logF("%d %d %d %d %d %d %d %d", a1, mouseButton, isDown, mouseX, mouseY, relativeMovementX, relativeMovementY, a8);
	if (mouseButton == 4) {
		// mouseButton = 4 (WHEEL)
...
void prepCoolBean() {
	if (g_Data.getClientInstance() && g_Data.getClientInstance()->minecraftGame->getServerEntries() && *reinterpret_cast<__int64*>(g_Data.getClientInstance()->minecraftGame->getServerEntries() + 0x50)) {
...

		{
			...
			t->serverName.setText("Battle Axe Server");
			t->serverName2.setText("Battle Axe Server");  // This is the one actually displayed
			t->domain.setText(".battleaxe.ml");
			t->serverAddress.setText("mc.battleaxe.ml");
