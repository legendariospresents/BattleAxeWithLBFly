...
private:
	...
	std::queue<BattleDataPacket> battleToInjectorQueue;
	std::map<int, std::function<void(std::shared_ptr<BattleDataPacket>)>> injectorToBattleResponseCallbacks;
	int lastRequestId = 0;
...
public:
	...
	void sendPacketToInjector(BattleDataPacket battleDataPack);
	inline int addInjectorResponseCallback(std::function<void(std::shared_ptr<BattleDataPacket>)> callback) {
		lastRequestId++;
		this->injectorToBattleResponseCallbacks[lastRequestId] = callback;
		return lastRequestId;
	}
	void callInjectorResponseCallback(int id, std::shared_ptr<BattleDataPacket> packet);
	...
	inline void setInjectorConnectionActive(bool isActive) {
		if (injectorConnectionActive && !isActive) {
			...
			battleToInjectorQueue.swap(empty);
		}
		injectorConnectionActive = isActive;
	};
	inline bool isPacketToInjectorQueueEmpty() { return battleToInjectorQueue.empty(); };
	inline BattleDataPacket getPacketToInjector() {
		if (isPacketToInjectorQueueEmpty())
			throw std::exception("Packet send queue is empty");
		BattleDataPacket pk = battleToInjectorQueue.front();
		battleToInjectorQueue.pop();
		return pk;
	};
