...
void GameData::sendPacketToInjector(BattleDataPacket battleDataPack) {
	if (!isInjectorConnectionActive())
		throw std::exception("Injector connection not active");
	if (battleDataPack.dataArraySize >= 3000) {
		logF("Tried to send data packet with array size: %i %llX", battleDataPack.dataArraySize, battleDataPack.data.get());
		throw std::exception("Data packet data too big");
	}

	battleToInjectorQueue.push(battleDataPack);
}
void GameData::callInjectorResponseCallback(int id, std::shared_ptr<BattleDataPacket> packet) {
	if (this->injectorToBattleResponseCallbacks.find(id) == this->injectorToBattleResponseCallbacks.end()) {
		logF("No response callback for request with id=%i!", id);
		return;
	}
	this->injectorToBattleResponseCallbacks[id](packet);
	this->injectorToBattleResponseCallbacks.erase(id);
}
